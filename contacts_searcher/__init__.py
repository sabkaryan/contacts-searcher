...
from contacts_searcher.domain.contact_repository import ContactRepository
...
if __name__ == "__main__":
    ...
    for inn in inn_list_gen:
        ...
        logging.info(f"Found {len(links)} links for inn={inn}")
    ...
    sanitized_res = sanitizer_service.clear_duplicates(res)
    stop = perf_counter()

    output_data_source = CSVDataSource("../output.csv")
    contact_repository = ContactRepository(output_data_source)
    contact_repository.write_all(sanitized_res)

    logging.info(f"Done in {stop - start} seconds")
